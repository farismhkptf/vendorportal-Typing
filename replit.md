...
-   **Admin Console** (`client/src/pages/admin/`): `index.tsx` (~194 lines orchestrator) imports 15 self-contained tab/section components (companies-tab, centers-tab, staff-tab, services-tab, job-types-tab, vendors-tab, accounts-tab, settings-section, api-keys-tab, attestation-services-tab, monthly-sheet-section, import-export-section, login-audit-tab, password-reset-requests-tab, change-log-tab, workdrive-backup-section, deletion-requests-tab).
...
-   **Appointments** (`client/src/pages/appointments/`): `index.tsx` (~170 lines) orchestrates 8 components + 3 custom hooks in `components/` (appointment-card, ready-to-schedule-card, appointment-dialogs, appointment-stats, appointment-calendar, appointment-toolbar, appointment-list-sections + use-appointments-data, use-appointment-actions, appointment-messages-handler hooks + types). `schedule-medical.tsx` (~289 lines) and `schedule-eid.tsx` (~303 lines) share 4 extracted components (schedule-shared-types, schedule-wo-selector, schedule-appointment-form, schedule-confirmation-step).
