...
-   **Work Order Command Center UX**: The app follows a Work Order-centric design where the WO detail page is the primary action hub. Key UX features:
    - **Pipeline Progress Bar** (`client/src/lib/pipeline-stage.ts`): Shared utility that computes pipeline stages (New → At Vendor → Ready to Schedule → Scheduled → Complete, plus Needs Attention) for Medical and EID tracks based on typing job and appointment statuses. Used across WO detail, WO list, and dashboard.
    - **WO Detail**: Shows a dual-track pipeline bar, smart "Next Action" banner with context-aware suggestions, and expanded typing job cards with inline vendor results, documents, comments, and action buttons (Deliver, Abort). Tabs are controlled and switch automatically based on next-action clicks.
