  ...
  return (
    <Dialog {...props}>
      <DialogPrimitive.Portal>
        <DialogPrimitive.Overlay className="spotlight-overlay fixed inset-0 z-50" />
        <DialogPrimitive.Content className="spotlight-dialog fixed left-[50%] translate-x-[-50%] z-50 overflow-hidden p-0 border-0 shadow-none bg-transparent w-[680px] top-[32%] rounded-2xl gap-0">
          <VisuallyHidden>
...
>(({ className, ...props }, ref) => (
  <div className="flex items-center px-4" cmdk-input-wrapper="">
    ...
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "flex h-12 w-full rounded-md bg-transparent py-3 text-base outline-none placeholder:text-muted-foreground/40 disabled:cursor-not-allowed disabled:opacity-50",
        className
      )}
      {...props}
    />
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.List
    ref={ref}
    className={cn("max-h-[400px] overflow-y-auto overflow-x-hidden scroll-smooth", className)}
    {...props}
  />
))
...
>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex cursor-default gap-2.5 select-none items-center rounded-xl px-3 py-2.5 text-sm outline-none transition-all duration-150 data-[disabled=true]:pointer-events-none data-[selected='true']:bg-accent/80 data-[selected=true]:text-accent-foreground data-[disabled=true]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
      className
    )}
    {...props}
  />
