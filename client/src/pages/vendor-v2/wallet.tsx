...
const ENTRY_ICONS: Record<string, { icon: typeof ArrowUpCircle; color: string; bg: string }> = {
  Topup: { icon: ArrowUpCircle, color: "text-emerald-400", bg: "bg-emerald-500/15" },
  Debit: { icon: ArrowDownCircle, color: "text-red-400", bg: "bg-red-500/15" },
  Reversal: { icon: RotateCcw, color: "text-blue-400", bg: "bg-blue-500/15" },
  Adjustment: { icon: Settings2, color: "text-amber-400", bg: "bg-amber-500/15" },
};
  ...
  return (
    <div className="max-w-2xl mx-auto pt-2 pb-4">
      <GlassCard className="p-6 mb-6 text-center" data-testid="card-v2-balance">
        ...
        <p className="text-4xl font-bold text-white tracking-tight tabular-nums" data-testid="text-v2-wallet-balance">
          {balance.toLocaleString()} <span className="text-lg font-normal text-white/40">AED</span>
        </p>
        ...
        <div className="flex items-center justify-center gap-6 mt-5">
          <div className="text-center" data-testid="stat-total-in">
            <div className="flex items-center gap-1.5 justify-center mb-0.5">
              ...
              <span className="text-xs text-white/40">Received</span>
            </div>
            <p className="text-sm font-semibold text-emerald-400 tabular-nums">{totalIn.toLocaleString()} AED</p>
          </div>
          <div className="h-8 w-px bg-white/10" />
          <div className="text-center" data-testid="stat-total-out">
            <div className="flex items-center gap-1.5 justify-center mb-0.5">
              <ArrowDownCircle className="h-3.5 w-3.5 text-red-400" />
              <span className="text-xs text-white/40">Debited</span>
            </div>
            <p className="text-sm font-semibold text-red-400 tabular-nums">{totalOut.toLocaleString()} AED</p>
          </div>
        </div>
      </GlassCard>
              ...
              <div key={group.label}>
                ...
                <div className="space-y-1">
                  {group.items.map(tx => {
                    ...
                    return (
                      <div key={tx.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors" data-testid={`v2-transaction-${tx.id}`}>
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {tx.jobInfo?.woNumber && (
                              <span className="text-[11px] text-white/30">{tx.jobInfo.woNumber}</span>
                            )}
                            {tx.jobInfo?.jobCategory && (
                              <span className={`v2-status-badge text-[10px] ${tx.jobInfo.jobCategory === "EID" ? "bg-amber-500/20 text-amber-300 border border-amber-500/30" : "bg-teal-500/20 text-teal-300 border border-teal-500/30"}`}>
                                {tx.jobInfo.jobCategory}
                              </span>
                            )}
                          </div>
                          {(tx.note || tx.jobInfo?.applicantName) && (
                            <p className="text-xs text-white/30 truncate">{tx.note || tx.jobInfo?.applicantName}</p>
                          )}
                        </div>
